    ...
    <>
      ...
      <div className="flex justify-between items-center px-3 py-3 bg-white">
        ...
        <div className="flex items-center">
          <h2 className="text-base font-medium text-[#4D4D4D]">
            湘信政参
          </h2>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center">
            <div className={`w-1.5 h-1.5 rounded-full ${loading || isProcessingChat ? 'bg-amber-500 animate-pulse' : 'bg-green-500'} mr-2`}></div>
            <span className="text-xs text-gray-400">{loading ? '研究中' : isProcessingChat ? '思考中' : '就绪'}</span>
          </div>
      ...
      <div 
        ...
        className="flex-1 overflow-y-auto py-2 px-2 custom-scrollbar bg-[#EEF0F4]"
      >
        {/* Status message - conditional on research state */}
        <div className="mb-4">
          <div className="p-3 bg-white rounded-md shadow-sm">
            <div className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-md bg-[#238580] flex items-center justify-center flex-shrink-0 text-white">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="text-[#4D4D4D] text-sm">
                {loading ? (
                  <p>正在进行研究... 完成后我会分析结果。</p>
                ) : (
                  <p>我已分析所有研究结果，可以回答您关于这份报告的任何问题。有什么可以帮您的吗？</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="py-3 px-2 bg-white">
        ...
        {isStopped && (
          <div className="text-center p-2 text-[#4D4D4D] bg-white rounded-md text-sm">
            研究已停止。开始新研究以继续对话。
          </div>
        )}
      </div>
